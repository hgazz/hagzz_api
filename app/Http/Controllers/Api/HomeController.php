...
class HomeController extends Controller
{
    ...
    public function home()
    {
        ...
        $academies = $academies = !auth()->check() ? PartnerResource::collection(Academies::with('sports')->select(['id','commercial_name','logo'])->get()) : PartnerResource::collection(Academies::with('sports')->select(['id','commercial_name','logo'])->whereHas('addresses.country',function($q){$q->where('country_id',auth('api')->user()->country_id);})->limit(6)->inRandomOrder()->get(['id','commercial_name','logo'])) ;
